""" Шифр пар -- является шифром замены.
    Есть привязка между двумя символами. Eсли в тексте попадается
    символ A, он заменяется на B (A → B), но это также действует и в обратную
    сторону (B → A).)"""
