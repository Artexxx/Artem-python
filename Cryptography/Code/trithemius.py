"""Шифр Тритемиуса – улучшенный шифр Цезаря в котором ключём является не
    число, а какая-либо функция. В отличии от шифра Цезаря не подвержен
    частотному криптоанализу, т.к. шифрует сообщение по индексам"""
