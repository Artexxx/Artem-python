...
try:
    vodaonline = requests.post('https://www.vodaonline.ru/local/components/shantilab/feedback.form/ajax.php',
                               data={'sessid': '*', 'NAME': _name, 'PHONE': _phoneVodaonline}, proxies=proxies)
...
try:
    yurmoscow = requests.post('https://yur-moscow.ru/ajax_call_me.php',
                              data={'param1': _phone, 'param3': _text, 'param2': _name}, proxies=proxies)
...
try:
    bukvaprava = requests.post('https://bukvaprava.ru/wp-admin/admin-ajax.php',
                               data={'text_quest_banner': _text, 'name': _name, 'city': 'Москва',
                                     ...
                                     'form_page': 'https://bukvaprava.ru/', 'referer': '', 'action': 'ajax-lead'},
                               proxies=proxies)
...
try:
    yuristonline = requests.post('https://www.yurist-online.net/lead_question',
                                 data={'region': '27', 'question': _text, 'name': _name, 'phone': _phone,
                                       'email': '', 'partner_id': '13'}, proxies=proxies)
...
try:
    rossovet = requests.post('https://rossovet.ru/qa/msgsave/save',
                             data={'name': _name, 'comment': _text, 'city': 'Москва',
                                   'phoneprefix': '(' + _phone[1:4] + ')', 'phone': _phone[4:11], 'partnerID': '10',
                                   'ref': 'https://yandex.ru/clck/', 'number1': '7', 'number2': '8',
                                   'checkcode': '15'}, proxies=proxies)
...
try:
    epleads = requests.post('https://epleads.ru/gate/api.php',
                            data={'question': _text, 'region': 'Москва', 'first_last_name': _name, 'phone': _phone,
                                  'ofrid': '1', 'wid': '3', 'presetid': '4',
...
try:
    pravonedv = requests.post('https://pravonedv.ru/proxy_8d34201a5b.php?a___=1',
                              data={'email': _email + '@mail.ru', 'phone': _phoneVodaonline,
                                    'location': 'Москва, Россия', 'name': _name, 'offer': '0',
...
try:
    rdftgbhnj = requests.post(
        ...
        data={'_wpcf7': '295', '_wpcf7_version': '5.0.5', '_wpcf7_locale': 'ru_RU',
              '_wpcf7_unit_tag': 'wpcf7-f295-o2', '_wpcf7_container_post': '0', 'text-838': _name,
              'tel-231': _phone, 'textarea-472': _text, 'hidden-278': 'Главная'}, proxies=proxies)
...
try:
    gurist = requests.post('http://www.gurist.ru/wp-json/contact-form-7/v1/contact-forms/3591/feedback',
                           data={'_wpcf7': '3591', '_wpcf7_version': '5.0', '_wpcf7_locale': 'ru_RU',
                                 ...
                                 'your-name': _name, 'tel-147': _text}, proxies=proxies)
...
try:
    advokatmakeev = requests.post('https://advokatmakeev.ru/form.php',
                                  data={'oname': _name, 'otel': _phoneVodaonline, 'omail': '', 'omess': _text,
                                        'otype': '1'}, proxies=proxies)
...
try:
    mkamsk = requests.post('http://mkamsk.ru/apply_auto_form',
                           data={'Form[9]': _name, 'Form[12]': _phone, 'Form[11]': _text,
                                 'url': 'http://mkamsk.ru/', 'check': 'check'}, proxies=proxies)
...
try:
    pravosfera = requests.post('http://pravo-sfera.ru/auxpage_zayavk/',
                               data={'c_name': _name, 'c_tel': _phoneVodaonline, 'quest': _text,
                                     'uest_go': 'Задать вопрос'}, proxies=proxies)
...
try:
    uristexpert24 = requests.post('https://urist-expert24.ru/send-lead/',
                                  data={'name': _name, 'phone': _phoneVodaonline,
                                        'form-name': 'Заказ обратного звонка'}, proxies=proxies)
...
try:
    lawdivorce = requests.post('http://law-divorce.ru/wp-admin/admin-ajax.php',
                               data={'ip_address': '', 'ip_country': '', 'ip_region': '', 'ip_city': '', 'url': '',
                                     'action': 'lld_send_lead', 'text': _text, 'name': _name,
                                     'telephone': '+' + _phoneBukvaprava, 'city': 'Москва'}, proxies=proxies)
    ...
    print(Fore.RED + 'Ошибка')
try:
    gosurist = requests.post('http://www.gos-urist.ru/send.php',
                             {'name': _name, 'code': _phone[1:4], 'phone': _phone[4:11], 'issue': _text},
                             proxies=proxies)
