...
class setAndDict():
    ...
    _dict = {key: _dict[key] for key in _dict.keys() - {"ZZZ", "XXX"}}

    """ Найти max|min значение в словаре"""
    _dict = {"key1": 30, "key2": 20, "key3": 10, }
    min(_dict, key=lambda key: _dict[key]) #=> 'key3'
    max(zip(_dict.values(), _dict.keys())) #=> (30, 'key1')
