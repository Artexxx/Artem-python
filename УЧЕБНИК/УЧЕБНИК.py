...
class regular_expression(): #регулярные выражения - поиск нужной инфы внутри текстового документа.
    # cначала с помощью рег.фуражений описываем шаблон, а потом можем проверить подходитли данная строка под наш шаблон.
    ...
    re.sub(pattern,"ass" ,str,k) # заменить все вхождения подстрок подходящ под данный шаблон чем-то ->'ass,ass,ass'k раз

    #[] -- можно указать множество подходящих символов /[a-zA-Z]-все буквы англ алфавита/
    # , ^ $ * + ? {} [] \ () -- метасимволы
    #\d - [0-9] -- Цифры
    #\D - [^0-9] -- Не цифры
    #\s - [ \t\n\r\f\v] -- пробельные символф
    #\S - [^ \t\n\r\f\v]
...
class shutil():"модуль для копировния и перемещения файлов"
    ...
    shutil.copy("porn.json/text.txt","porn.json/result.txt") , """  копирует файл  """
...
class sys():
    ...
    for line in sys.stdin: """Считать все строки по одной из стандартного потока ввода вы можно так"""
        line = line.strip()
