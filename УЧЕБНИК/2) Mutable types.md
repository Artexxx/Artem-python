...
Рассмотрим сначала то, как выглядит структура отдельно взятого элемента словаря в [pycore_dict.h](https://github.com/python/cpython/blob/main/Include/internal/pycore_dict.h):
```c
typedef struct {
    Py_hash_t me_hash;
    PyObject *me_key;
