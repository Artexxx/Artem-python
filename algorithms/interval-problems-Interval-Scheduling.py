...
def countIntersectionInterval(intervals):
    """
    Возвращает максимальное число интервалов без какого-либо перекрытия.
...
def eraseOverlapIntervals(intervals):
    """
    Возвращает минимальное количество  интервалов, которые надо удалить, чтобы остались интервалы без какого-либо перекрытия.
...
def min_num_taxis(requests):
    ...
    """
    zeros = np.zeros(100000)  # FIXME
    for r in requests:
        zeros[r[0]:r[1] + 1] += 1
    return max(zeros)


def min_num_taxis_optimized(requests):
    """
